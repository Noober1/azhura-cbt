/**
 * Azhura CBT Console — Student import modal (#70).
 *
 * Multi-state flow:
 *  idle       → mode toggle (Import/Sync) + drag-drop zone + template download
 *  loading    → dry-run in progress
 *  preview    → show row results + sync summary; confirm or cancel
 *  confirming → executing upsert/delete
 *  done       → show result + optional error report download
 */

import { useRef, useState } from "react";
import { studentsImportApi } from "../../lib/students-import-api";
...
export function StudentImportModal({ open, onClose, onImported }: StudentImportModalProps) {
  ...
  const [mode, setMode] = useState<"import" | "sync">("import");
  ...
  async function runDryRun(file: File) {
    ...
    setStep("loading");
    try {
      const data = await studentsImportApi.dryRun(file, mode);
      setPreview(data);
      setStep("preview");
    } catch (err) {
  ...
  async function handleConfirm() {
    ...
    setStep("confirming");
    try {
      const res = await studentsImportApi.confirm(preview.sessionId);
      setResult(res);
      setStep("done");
      onImported();
      const parts: string[] = [];
    ...
    >
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              leadingIcon={<DownloadIcon className="size-4" />}
              onClick={() =>
                studentsImportApi.downloadTemplate("xlsx").catch(() => toast.error("Gagal mengunduh template."))
              }
            >
              .xlsx
            </Button>
            <Button
              ...
              leadingIcon={<DownloadIcon className="size-4" />}
              onClick={() =>
                studentsImportApi.downloadTemplate("csv").catch(() => toast.error("Gagal mengunduh template."))
              }
            >
              .csv
            </Button>
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-faint">Format: .xlsx atau .csv · Maks. 1.000 baris</p>
            </div>
          </div>
          <input
            ref={inputRef}
      ...
      {(step === "preview" || step === "confirming") && preview && (
        <div className="space-y-4">
            ...
            <div className="rounded-md border border-warn-wash bg-warn-wash/40 px-3 py-2 text-sm">
              {(preview.toDelete ?? 0) > 0 && (
                <p className="text-warn-strong">
                  ⚠️ <strong>{preview.toDelete} siswa</strong> akan dihapus (tidak ada di file).
                </p>
              )}
              {(preview.skippedDelete ?? 0) > 0 && (
                <p className="text-faint">
                  {preview.skippedDelete} siswa dilewati (punya riwayat ujian, tidak bisa dihapus).
                </p>
              )}
              {(preview.toDelete ?? 0) === 0 && (preview.skippedDelete ?? 0) === 0 && (
                <p className="text-faint">Tidak ada siswa yang akan dihapus.</p>
              )}
            </div>
          )}
          ...
          {errorCount > 0 && (
            <p className="text-sm text-faint">
              Hanya baris valid yang akan diproses. Baris error dilewati.
            </p>
          )}
      ...
      {step === "done" && result && (
        <div className="space-y-4 py-2">
          <div className="flex flex-wrap gap-2">
            {result.inserted > 0 && (
              <Badge tone="positive">{result.inserted} siswa baru ditambahkan</Badge>
            )}
            {result.updated > 0 && (
              <Badge tone="accent">{result.updated} siswa diperbarui</Badge>
            )}
            {result.deleted > 0 && (
              <Badge tone="danger">{result.deleted} siswa dihapus</Badge>
            )}
            {result.skipped > 0 && (
              <Badge tone="neutral">{result.skipped} dilewati (riwayat ujian)</Badge>
            )}
            {errorCount > 0 && <Badge tone="warn">{errorCount} baris error dilewati</Badge>}
          </div>
          {errorCount > 0 && (
            <p className="text-sm text-faint">
